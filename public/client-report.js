...
function reportView(data,{shared=false}={}){
  const card=E('section','client-report-card');
  ...
  copy.append(
    ...
    E('p','story-summary',data.report.executiveSummary),
  );
  actions.append(E('span','pill',data.includePrivate?T('Private workspace report','Báo cáo private trong workspace'):T('Public evidence report','Báo cáo bằng chứng public')));
  if(!shared){
  ...
  if(data.evidence?.length){const details=E('details','evidence-details'),summary=E('summary','evidence-summary',T(`Evidence · ${data.evidence.length} supporting sources`,`Bằng chứng · ${data.evidence.length} nguồn hỗ trợ`)),list=E('div','evidence-list');data.evidence.forEach(item=>{const a=E('a','evidence');a.href=item.url;a.target='_blank';a.rel='noreferrer';const title=E('span','evidence-title');title.append(E('strong','',item.title),E('small','',`${item.repo} · ${item.date}${item.visibility==='private'?' · private':''}`));a.append(E('span','evidence-id',item.id),title);list.append(a)});details.append(summary,list);card.append(details)}
  return card;
}

function isPrivateAnalysis(){return $('#report')?.dataset.privateMode==='true'||document.body.dataset.privateAnalysis==='true'}

async function latestSnapshotId(){
  const currentUsername=$('#username')?.value.trim();if(!currentUsername)return null;
  const currentDays=$('#days')?.value||'30',lang=locale(),includePrivate=isPrivateAnalysis();
  const url=new URL('/api/history',location.origin);url.searchParams.set('username',currentUsername);url.searchParams.set('days',currentDays);url.searchParams.set('locale',lang);url.searchParams.set('includePrivate',String(includePrivate));
...
async function renderSharedRoute(){
  ...
  $('.hero')?.classList.add('hidden');$('.example-preview')?.classList.add('hidden');$('.value-strip')?.classList.add('hidden');$('#status')?.classList.add('hidden');
  ...
  try{
    ...
    if(root){root.replaceChildren();const intro=E('div','shared-report-intro');intro.append(E('span','eyebrow','Dev30 stakeholder briefing'),E('p','muted',`@${data.username} · ${data.days} ${data.locale==='vi'?'ngày':'days'} · ${new Date(data.createdAt).toLocaleString()}`));root.append(intro,reportView(data,{shared:true}))}
...
if(!(await renderSharedRoute())){
  const observer=new MutationObserver(()=>attachGenerator());const reportRoot=$('#report');if(reportRoot)observer.observe(reportRoot,{childList:true,subtree:true});attachGenerator();
}
