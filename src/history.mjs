...
function seriesKey({ username, days, includePrivate, locale }) {
  return [String(username || '').toLowerCase(), Number(days) || 30, includePrivate ? 'private' : 'public', locale === 'vi' ? 'vi' : 'en'].join(':');
}
...
export function buildSnapshot({ dataset, payload, locale = 'en', generatedAt = new Date().toISOString() }) {
  const snapshot = {
    ...
    includePrivate: Boolean(dataset.collector.includePrivate),
...
export async function listSnapshots({ username, days = 30, includePrivate = false, locale = 'en', limit = 12, filePath = historyFilePath() }) {
  const store = await readStore(filePath);
  const key = seriesKey({ username, days, includePrivate, locale });
  ...
  for (const [name, now] of after) {
    ...
    if (commitDelta || prDelta) {
      repoChanges.push({
        ...
        commitDelta,
        prDelta,
        currentCommits: now.commits || 0,
        currentPullRequests: now.pullRequests || 0,
      });
    }
  }
...
export function deterministicDeltaNarrative(delta, locale = 'en') {
  ...
  if (delta.focus.changed) {
    highlights.push({
      ...
      text: vi
        ? `Trọng tâm chuyển từ ${delta.focus.from.repo || 'chưa rõ'} sang ${delta.focus.to.repo || 'chưa rõ'}.`
        : `Main focus moved from ${delta.focus.from.repo || 'unknown'} to ${delta.focus.to.repo || 'unknown'}.`,
    });
  }
  for (const change of delta.repoChanges.slice(0, 4)) {
    highlights.push({
      ...
      repo: change.repo,
      text: vi
        ? `${change.repo}: số commit quan sát thay đổi ${change.commitDelta >= 0 ? '+' : ''}${change.commitDelta}, PR ${change.prDelta >= 0 ? '+' : ''}${change.prDelta}.`
        : `${change.repo}: observed commits changed ${change.commitDelta >= 0 ? '+' : ''}${change.commitDelta}, PRs ${change.prDelta >= 0 ? '+' : ''}${change.prDelta}.`,
    });
  }
  const headline = vi ? 'So sánh snapshot mới với lần phân tích trước.' : 'Comparing the new snapshot with the previous analysis.';
  const summary = vi
    ? `Phát hiện ${delta.newRepos.length} repo mới trong cửa sổ, ${delta.inactiveRepos.length} repo không còn xuất hiện và ${delta.newWorkUnits.length} đơn vị công việc mới được quan sát.`
    : `Observed ${delta.newRepos.length} newly appearing repos, ${delta.inactiveRepos.length} repos no longer in the window, and ${delta.newWorkUnits.length} new work units.`;
